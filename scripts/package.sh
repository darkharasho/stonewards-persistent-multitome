...
plugin = re.search(r'PluginVersion = "([^"]+)"', (root / "src/PersistentMultiTome/Plugin.cs").read_text())
csproj = re.search(r"<Version>([^<]+)</Version>", (root / "src/PersistentMultiTome/PersistentMultiTome.csproj").read_text())
...
if not csproj or csproj.group(1) != v:
    errors.append(f"PersistentMultiTome.csproj Version is {csproj and csproj.group(1)}, manifest is {v}")
...
zipfile="$root/dist/darkharasho-PersistentMultiTome-$version.zip"

dotnet build "$root/src/PersistentMultiTome/PersistentMultiTome.csproj" -c Release

rm -rf "$stage" "$zipfile"
mkdir -p "$stage/plugins"
cp "$manifest" "$root/thunderstore/icon.png" "$root/thunderstore/README.md" "$root/CHANGELOG.md" "$stage/"
cp "$root/src/PersistentMultiTome/bin/Release/PersistentMultiTome.dll" "$stage/plugins/"
